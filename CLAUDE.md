...
- **firma-sidecar** — The enforcement proxy binary. Four top-level modules:
  ...
  - `normalizer` — Maps raw HTTP requests to canonical `ExecutionEnvelope` with normalized `intent.action_class` from a 27-class registry (15 FEP v0.1 + 12 task-017 GitHub additions). `intent.resource` is a `BTreeMap<String, String>` with conventional keys `host`, `path`, and optionally `provider` (only attached when the request host exact-matches a known allowlist, e.g. `api.github.com` → `provider="github"`). Fail-closed: unclassifiable protected actions → DENY.
...
startup via `MappingTable::from_config`. Two config knobs on
`[mapping]` (see `crates/firma-sidecar/src/enforcement/config.rs`):
...
validated together. Duplicate `(method, host, path)` tuples across merged files
fail at startup (fail-closed).

Shipped mapping files live under `crates/firma-sidecar/config/mappings/`:

| File          | Covers                                                |
|---------------|-------------------------------------------------------|
| `github.toml` | 44 GitHub REST endpoints → 12 action classes (task 017) |
