...
A dedicated scaffolding intent sets up the Cargo workspace with all four crates (`openauthority-core`, `openauthority-proto`, `opensidecar`, `openauthority-authority`) before any real implementation. This establishes crate boundaries, dependency graph, and CI from the start — avoiding painful restructuring later.
...
|---|--------|---------|------------|
| 001 | Project scaffolding | Cargo workspace with `openauthority-core`, `openauthority-proto`, `opensidecar`, `openauthority-authority` stubs, CI pipeline, key traits with mock impls | — |
| 002 | Core types & shared library | Real `openauthority-core` — capability tokens (PASETO v4 / JWT RS256), Cedar wrapper, error types, storage traits, capability library | 001 |
| 003 | gRPC protocol & wire contract | `openauthority-proto` — Protobuf service definitions (AuthorityService RPCs: IssueCapability, WatchPolicyBundle, WatchRevocations), messages (ExecutionEnvelope, CapabilityToken), generated code | 001 |
...
| 005 | Mini Authority service | Real `openauthority-authority` — Cedar policy loader from `.cedar` files, IssueCapability RPC, WatchPolicyBundle streaming, WatchRevocations streaming, capability token generation | 002, 003 |
| 006 | Sidecar proxy & enforcement | Real `opensidecar` — Pingora HTTP proxy, Stage 1 (token parse, sig verify, revocation bloom filter), Stage 2 / CEE (context build, Cedar eval, budget/scope/threshold), generic HTTP connector, credential injector, audit emitter | 002, 003 |
| 007 | OpenAuthority Run sandbox launcher | `openauthority run` generic wrapper — sandboxed agent launch, mandatory sidecar routing, DNS confinement, built-in `generic` + `codex` profiles | 006 |
| 008 | Local dev mode | `openauthority dev up` orchestration — wires Mini Authority (:50051) + Sidecar (:8080) + Example Agent, Cedar hot-reload, audit to stdout, end-to-end demo in under 2 minutes | 004, 005, 006, 007 |

## V1 Scope Exclusions

Decisions made during inception that are explicitly out of scope for V1:

- **Capability Library in Go/Python/TypeScript**: V1 is Rust-only (`openauthority-core`). Agents interact via HTTP_PROXY — no agent-side SDK needed. Agent-side token inspection is a post-V1 nice-to-have.
...
- **Sandbox boundary is now required for complete coverage**: FIR-60 updated architecture direction on 2026-04-23. `openauthority-run` (FIR-61) moves the hard boundary to sandboxed execution plus mandatory sidecar routing so shell/subprocess/browser-originated traffic is structurally governed.
...
How intents map to components from `openauthority_oss_component_reference.md`:
...
| Section 4 — OpenAuthority Sidecar (interceptor, execution envelope, Stage 1, Stage 2/CEE, local state, audit emitter, credential injector) | 006 |
...
| Section 10 — Local Dev Mode (`openauthority dev up`) | 008 |
