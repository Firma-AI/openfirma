# **OpenAuthority AI — Open Source Release**

`OSS Runtime Realization of FEP v0.1.2`

| This document describes every component included in the OpenAuthority AI open-source release (Apache 2.0). It is the primary reference for the team building on top of the OSS package. It explicitly calls out V1 scope boundaries, failure modes, and what is intentionally excluded. |
| :---- |
...
| Entities |  |
| :---- | :---- |
| **Agent** | The full orchestration layer: LLM model \+ tool executor \+ session state. OpenAuthority treats the agent as the principal requesting capabilities. Not synonymous with “LLM” — the LLM is one component inside the agent. |
...
| **Core Concepts** |  |
| **Capability** | A scoped, time-bounded permission granted to an agent for a specific action class (e.g. “write to Postgres table orders, max 50 rows, expires in 10 min”). Issued by OpenAuthority Authority before execution begins. Defines the permission perimeter. The Sidecar enforces constraints within it but cannot extend or override it. |
...
| **FEP Protocol Primitive—ExecutionEnvelope** | The core protocol unit of the OpenAuthority system. The protocol object wrapping each outbound call: intent, capability token reference, call metadata, and provenance. Each outbound call is represented as a distinct Execution Envelope evaluated independently by the Sidecar. Every request is evaluated, enforced, and audited as an Execution Envelope. Treated as immutable once created — any enrichment should produce a derived structure. Defined semantically by the FEP Specification v0.1. |
...
| **Runtime Mechanics** |  |
| **Hot Path** | Any outbound call originating from the agent: tool execution, external API calls, database queries, LLM API calls. Every such call passes through the Sidecar (Stage 1 \+ Stage 2\) before reaching the target. No round-trip to OpenAuthority Authority on the hot path — all evaluation is local. |
...
| OpenAuthority Sidecar (this OSS package) | Sandbox / host runtime |
| :---- | :---- |
...
| Can be replaced by config change (Mini Auth → FA) | Orthogonal to OpenAuthority; managed by the deployment platform |
...
Each component in the OpenAuthority stack prevents a specific class of threats. This section maps components to threats to make the security model explicit and auditable.
...
| **Sandbox / runtime** | Prevents unsafe code execution and runtime escape outside the application-layer policy boundary. Orthogonal to OpenAuthority; managed by the deployment platform. |
...
Mini Authority is the reference implementation of the OpenAuthority Authority interface. Its sole purpose is to make the full OSS stack runnable locally without any cloud service. It is intentionally minimal: no risk engine, no trust graph, no multi-tenancy.
...
| Mini Authority is labeled "reference impl · dev / test / demo · not for production". Never deploy it as a real authority endpoint. In production, swap it for OpenAuthority Authority via a single config-file change: openauthority.authority: FA\_URL. The Sidecar binary does not change. |
| :---- |

## **4\. OpenAuthority Sidecar**

OpenAuthority Sidecar is the primary OSS component. It is the enforcement layer between an agent and the outside world. Every outbound call passes through it. The Sidecar is a single statically-linked binary with no persistent database; all state is in-memory and re-populated from Authority streams on restart.
...
Runnable reference applications demonstrating the full OpenAuthority execution flow end-to-end. Not production-grade agents; designed to be readable and copy-paste friendly. Both connect to the Sidecar via HTTP proxy (HTTP\_PROXY=http://localhost:8080) with no code-level integration required.
...
Same three scenarios using axios and native fetch. Compatible with LangChain.js and Vercel AI SDK. Structurally identical to the Python agent, confirming OpenAuthority is language-agnostic at the agent layer. Both agents log the enforcement decision and latency for each call.
...
| `Translate Execution Envelope → target protocol` | Converts the OpenAuthority-internal Execution Envelope to the target system’s native protocol (HTTP, gRPC, DB query, tool call payload, etc.). |
| :---- | :---- |
...
| :---- | :---- |
| Managed by OpenAuthority (issued, validated, expired, revoked) | Managed by the agent runtime and application |
...
* Getting started guide — zero to running end-to-end demo with openauthority dev up.

## **10\. Local Dev Mode  ·  openauthority dev up**
...
| :---- | :---- |
| **`Dynamic risk engine`** | No real-time risk scoring. V1 risk is a static attribute threshold-check. A live risk engine is a OpenAuthority Authority (production) capability. |
...
| **`Compliance-grade audit backend`** | The audit emitter produces signed events. It does not provide a tamper-evident store, replay index, or compliance reporting. OpenAuthorityChain covers this in production. |
...
| For production deployments: replace Mini Authority with OpenAuthority Authority (config-only swap), route audit events to OpenAuthorityChain, and attach the F-Control Plane for policy authoring and bundle distribution. The Sidecar binary is identical in both environments. |
| :---- |
