...
**Classification:** Internal — Firma AI Core Team

# **1\.   Purpose**

The **Firma Execution Protocol (FEP)** defines the semantic layer through which an AI agent requests, governs, and executes actions on external systems. FEP operates above the transport layer: it is agnostic to whether the underlying wire is HTTP, gRPC, or a local in-process call.
...
| **`FEP`** | Firma Execution Protocol. The semantic L7 protocol defined in this document. |
...
* Compliance-grade audit export backend (Firma Chain).  
...
These capabilities are architectural commitments — they are reflected in the protocol schema and the component interfaces — but they are not activated in the OSS build. See the **Firma AI OSS Component Reference**, Section 13, for the complete V1 scope boundary.