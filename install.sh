...
install_binary() {
    ...
    if [ -n "$shim_path" ]; then
        libexec_dir="${INSTALL_DIR}/libexec/openfirma/secret-shims/$(uname -m)-unknown-linux-musl"
        run "mkdir -p '$libexec_dir'"
        run "mv '$shim_path' '${libexec_dir}/firma-secret-shim'"
        run "chmod 0755 '${libexec_dir}/firma-secret-shim'"
    fi
