...
use openauthority_core::OpenAuthorityEntityUid;
use openauthority_core::agent::AgentId;
use openauthority_core::policy::PolicyBundle;
use openauthority_core::session::SessionId;
use openauthority_core::token::CapabilityClaims;
use openauthority_core::token::paseto::PasetoV4Signer;
use openauthority_proto::RevocationEvent;
use openauthority_proto::openauthority::v1::authority_service_server::AuthorityService;
use openauthority_proto::openauthority::v1::{
...
pub(crate) fn evaluate_cedar_policy(
    ...
    let principal: cedar_policy::EntityUid =
        match OpenAuthorityEntityUid::Agent(agent_id.clone()).try_into() {
            Ok(uid) => uid,
            Err(e) => {
    ...
    let resource_entity: cedar_policy::EntityUid =
        match OpenAuthorityEntityUid::Resource(resource.to_string()).try_into() {
            Ok(uid) => uid,
            Err(e) => {
    ...
    for action in actions {
        let action_entity: cedar_policy::EntityUid =
            match OpenAuthorityEntityUid::Action(action.clone()).try_into() {
                Ok(uid) => uid,
                Err(e) => {
...
fn bundle_to_update(bundle: &PolicyBundle) -> PolicyBundleUpdate {
    PolicyBundleUpdate {
        bundle: Some(openauthority_proto::PolicyBundle {
...
mod tests {
    ...
    const FIRMA_SCHEMA: &str = include_str!("../../openauthority-authority/schema.cedarschema");

    fn openauthority_schema() -> Schema {
    ...
    fn evaluate_with_schema_valid_action() {
        let schema = openauthority_schema();
        let result = evaluate_cedar_policy(
            &permit_all(),
            &schema,
            &agent("agent_1"),
