...
/// Canonical `OpenAuthority` enforcement schema, embedded at compile time.
///
...
mod tests {
    ...
    fn extended_enforcement_context_parses() {
        ...
        let action_uid: EntityUid = "OpenAuthority::Action::\"communication.external.send\""
            .parse()
    ...
    #[test]
    fn schema_supports_openauthority_actions() {
        use cedar_policy::{
        ...
        for action in ACTIONS {
            let principal: EntityUid = "OpenAuthority::Agent::\"agent_test\""
                .to_string()
                .parse()
                .unwrap_or_else(|e| panic!("principal parse failed: {e}"));
            let action_uid: EntityUid = format!("OpenAuthority::Action::\"{action}\"")
                .parse()
                .unwrap_or_else(|e| panic!("action parse failed for '{action}': {e}"));
            let resource: EntityUid = "OpenAuthority::Resource::\"r\""
                .to_string()
                .parse()
