...
pub enum AbortReason {
    ...
    /// Sidecar blocked operation for security reasons after enforcement allowed the call.
    #[error("credential injection blocked")]
    CredentialInjectionBlocked,
...
impl AbortReason {
    ...
    pub const fn code(self) -> &'static str {
        match self {
            ...
            Self::CredentialInjectionBlocked => "CREDENTIAL_INJECTION_BLOCKED",
...
// Secret-extraction matcher representation lives in `firma-config-schema`
// (the single source of truth for config shape). Re-exported here so the
// canonical `firma_core::SecretMatcher` path — and every downstream consumer —
// stays unchanged.
pub use firma_config_schema::secret_matcher::{
    SecretJsonSelector, SecretJsonSelectorScope, SecretMatcher, SecretNameSource,
};
    ...
    fn test_abort_reason_code_all_variants() {
        ...
        for reason in AbortReason::iter() {
            let expected = match reason {
                ...
                AbortReason::CredentialInjectionBlocked => "CREDENTIAL_INJECTION_BLOCKED",
