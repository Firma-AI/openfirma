...
enum CrankUntilLoopOutcome {
    ...
    TimedOut {
        outcome: Option<ControlCrankOutcome>,
    },
}
...
impl<'a> HeadlessRunner<'a> {
    ...
    /// was ready.
    ///
    /// A quit outcome fails immediately. Tests call this helper while waiting
    /// for a state transition that should happen before shutdown.
...
fn format_last_crank_outcome(outcome: Option<ControlCrankOutcome>) -> String {
    outcome.map_or_else(|| String::from("none"), |outcome| format!("{outcome:?}"))
}
