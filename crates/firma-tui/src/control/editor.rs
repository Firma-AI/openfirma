...
fn platform_launch(path: &Path, editor: &Editor) -> EditorLaunch {
    let Some(command) = editor.command.as_ref() else {
        return EditorLaunch::new("notepad").args([path.as_os_str().to_owned()]);
    };

    if !windows_editor_needs_shell(command) {
        return EditorLaunch::new(command.clone()).args([path.as_os_str().to_owned()]);
    }
    ...
    EditorLaunch::new("cmd")
        ...
        .env(EDITOR_PATH_ENV, path.as_os_str().to_owned())
}
