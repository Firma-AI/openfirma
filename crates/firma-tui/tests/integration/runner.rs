...
#[test]
fn crank_until_returns_quit_when_quit_satisfies_condition() -> anyhow::Result<()> {
    let mut runner = HeadlessRunner::new(None, None);

    let outcome = runner.crank_until(
        &FakeTerminal::with_key(KeyCode::Char('q')),
        successful_editor(),
        firma_tui::control::App::should_quit,
        CRANK_TEST_TIMEOUT,
    )?;
...
#[test]
fn crank_until_reports_no_event_when_timeout_expires_before_cranking() -> anyhow::Result<()> {
    let mut runner = HeadlessRunner::new(None, None);
    ...
    assert_eq!(
        error.to_string(),
        "condition was not met within 0ns; last outcome: NoEvent"
    );
