...
///
/// Startup holds this transaction until it publishes complete state or finishes
/// rollback. Stop uses transactions around its initial snapshot and final
/// cleanup, with a generation lease connecting those phases. The operating
/// system releases the advisory lock if its owner exits unexpectedly.
...
impl StateTransaction {
    ...
    ///
    /// `None` means another process currently owns the transaction.
    ///
    /// # Errors
    ///
...
/// generation of runtime-state files from its predecessors and successors.
...
impl StackGeneration {
    ...
    /// Parse a persisted generation identity.
    ...
    /// Return the canonical text stored in the runtime-state directory.
...
/// generation still matches `stack.lock`. It does not authorize process
/// signalling; process ownership remains represented by `OwnedComponent`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl StateLease {
    ...
    /// `None` means another generation already owns the lock.
    ///
    /// # Errors
    ///
    ...
    ///
    /// `None` represents an absent lock or an empty lock created by an older
    /// version. Legacy state remains stoppable but does not gain a lease token.
    ///
    /// # Errors
    ///
    ...
    /// Return whether this lease belongs to the expected startup attempt.
    pub(crate) fn belongs_to(self, generation: StackGeneration) -> bool {
        self.generation == generation
    }
}

/// Write and flush a complete generation in a temporary file beside the lock.
...
/// Open the persistent coordination file shared by transaction operations.
