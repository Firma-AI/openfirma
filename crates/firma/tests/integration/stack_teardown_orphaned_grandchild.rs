...
fn unix_pgrp_kills_grandchild_after_leader_exits() {
    ...
    // The leader exits immediately on TERM while its grandchild ignores TERM.
    // Both remain in the process group identified by the leader's recorded PID.
    ...
    // Process death closes file descriptors before zombie reaping, so FIFO EOF
    // proves the orphaned descendant was killed without inspecting its PID.
    let deadline = Instant::now() + Duration::from_secs(5);
