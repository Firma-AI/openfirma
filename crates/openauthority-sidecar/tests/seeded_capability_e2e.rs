...
//! either `openauthority-authority issue` or `startup::capability` will trip
//! CI.
...
use openauthority_sidecar::config::CapabilitySeedConfig;
use openauthority_sidecar::startup::{build_token_verifier, load_capability_map};

/// Build the `openauthority-authority` binary and return the absolute path to
...
/// same package as the binary target; `openauthority-authority` lives in a
/// sibling crate.
fn build_authority_bin() -> PathBuf {
    ...
    let status = Command::new(&cargo)
        ...
        .status()
        .expect("cargo build openauthority-authority");
    assert!(
        status.success(),
        "cargo build openauthority-authority failed"
    );

    // `CARGO_MANIFEST_DIR` is `crates/openauthority-sidecar`; the workspace
    ...
    assert!(
        bin.exists(),
        "openauthority-authority binary missing at {}",
        bin.display()
    );
    bin
}
...
fn issued_token_seeds_capability_map_and_admits_stage1() {
    ...
    .unwrap();
    let schema_src = include_str!("../../openauthority-authority/schema.cedarschema");
    std::fs::write(policies.join("schema.cedarschema"), schema_src).unwrap();

    // Resolve (and ensure) the openauthority-authority binary. Cargo does not
    // set `CARGO_BIN_EXE_openauthority-authority` for tests outside that bin's
    ...
    let key_path = tmp.path().join("openauthority-authority.key");
