...
pub struct SidecarConfig {
    ...
    /// pre-issue tokens via `openauthority-authority issue` and list the
...
mod tests {
    ...
    fn test_unix_socket_mode_valid() {
        let config = SidecarConfig {
            interceptor: InterceptorConfig {
                mode: InterceptorMode::UnixSocket,
                socket_path: Some(PathBuf::from("/tmp/openauthority.sock")),
                ..InterceptorConfig::default()
            },
            ..SidecarConfig::default()
        };
...
[policy]
dir = "/etc/openauthority/policies"
authority_url = "https://authority.example.com"

[ca]
dir = "/etc/openauthority/ca"
...
[mapping]
rules_path = "/etc/openauthority/rules.toml"
...
wal_path = "/var/lib/openauthority/wal"
wal_max_bytes = 52428800
signing_key_path = "/etc/openauthority/audit.pem"
"#;
        let config: SidecarConfig =
        ...
        assert_eq!(
            config.policy.dir,
            PathBuf::from("/etc/openauthority/policies")
        );
        assert_eq!(
            config.policy.authority_url.as_deref(),
            Some("https://authority.example.com")
        );
        assert_eq!(config.ca.dir, PathBuf::from("/etc/openauthority/ca"));
        ...
        assert_eq!(
            ...
            Some(std::path::Path::new("/var/lib/openauthority/wal"))
        );
        assert_eq!(config.audit.wal_max_bytes, 52_428_800);
        assert_eq!(
            config.audit.signing_key_path.as_deref(),
            Some(std::path::Path::new("/etc/openauthority/audit.pem"))
        );
...
[interceptor]
mode = "unix_socket"
socket_path = "/tmp/openauthority.sock"
drain_timeout_secs = 10
"#;
        let config: SidecarConfig =
            toml::from_str(toml_str).unwrap_or_else(|e| panic!("parse failed: {e}"));
        ...
        assert_eq!(
            ...
            Some(std::path::Path::new("/tmp/openauthority.sock"))
        );
    }
}
