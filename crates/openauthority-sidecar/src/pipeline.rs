...
mod tests {
    ...
    async fn test_enforce_revoked_token_denied_through_pipeline() {
        struct RevokedStore;
        impl openauthority_core::RevocationStore for RevokedStore {
            fn is_revoked(
                &self,
                _token_id: &TokenId,
            ) -> Result<bool, openauthority_core::TokenError> {
                Ok(true)
            }
            fn add_revocation(
                &self,
                _token_id: &TokenId,
            ) -> Result<(), openauthority_core::TokenError> {
                Ok(())
            }
        }
    ...
    async fn test_enforce_sensitive_headers_stripped_in_allow() {
        ...
        if let EnforcementDecision::Allow { envelope, .. } = decision {
            if let openauthority_core::ActionParams::Http(ref params) = envelope.intent().params {
                assert!(
