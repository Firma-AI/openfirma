...
use openauthority_core::DenyReason;
...
impl HttpInterceptor {
    ...
    pub fn new(address: SocketAddr) -> Self {
        ...
        Self {
            ...
            handler: None,
            https_mitm_config,
            ca_dir: PathBuf::from("./openauthority-ca/"),
...
mod tests {
    ...
    async fn test_proxy_allows_valid_request_and_forwards_to_upstream() {
        ...
        let request = format!(
            "POST http://{host}/v1/chat/completions HTTP/1.1\r\n\
             Host: {host}\r\n\
             X-Openauthority-Session-Id: _test_\r\n\
             Content-Length: 2\r\n\
             \r\n\
             {{}}"
        );
    ...
    async fn test_proxy_connect_mitm_intercepts_and_applies_l7_deny() {
        ...
        let ca_cert_path = ca_tempdir.path().join("openauthority-ca.crt");
        for _ in 0..20 {
