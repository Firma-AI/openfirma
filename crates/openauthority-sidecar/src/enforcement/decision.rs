...
use openauthority_core::{CapabilityClaims, DenyReason, ExecutionEnvelope, InjectedCredentials};
    ...
    fn test_allow_helpers() {
        let decision = EnforcementDecision::Allow {
            ...
            envelope: Box::new(ExecutionEnvelope::new(
                openauthority_core::ExecutionIntent {
                    action_class: "filesystem.read".to_string(),
                    resource: openauthority_core::ExecutionIntent::resource_map_from("example.com"),
                    params: openauthority_core::ActionParams::Http(
                        openauthority_core::HttpParams {
                            method: openauthority_core::HttpMethod::GET,
                ...
                "token".to_string(),
                openauthority_core::ExecutionMetadata {
                    session_id: "sess".parse().expect("literal session id"),
                    agent_id: "agent".parse().expect("literal agent id"),
