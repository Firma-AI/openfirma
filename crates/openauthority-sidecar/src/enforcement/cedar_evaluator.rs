...
impl CedarPolicyEvaluator {
    ...
    fn evaluate_inner(
        ...
        let principal_uid: EntityUid = OpenAuthorityEntityUid::Agent(principal.clone())
            .try_into()
            .map_err(CedarEvaluatorError::EntityUidParse)?;
        let action_uid: EntityUid = OpenAuthorityEntityUid::Action(action.to_string())
            .try_into()
            .map_err(CedarEvaluatorError::EntityUidParse)?;
        let resource_uid: EntityUid = OpenAuthorityEntityUid::Resource(resource.to_string())
            .try_into()
            .map_err(CedarEvaluatorError::EntityUidParse)?;
...
impl PolicyEvaluation for CedarPolicyEvaluator {
    ...
    ///
    /// Entity UIDs are constructed via [`OpenAuthorityEntityUid`] to match the
...
namespace OpenAuthority {
    ...
    //
    // Run with: cargo test -p openauthority-sidecar payment_splitting

    const PAYMENT_SCHEMA: &str = r#"namespace OpenAuthority {
    entity Agent;
    entity Resource;
    type PaymentContext = {
        session_id: String,
