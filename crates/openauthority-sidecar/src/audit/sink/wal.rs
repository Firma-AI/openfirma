...
use openauthority_proto::openauthority::v1::audit_service_client::AuditServiceClient;
...
/// handle.
type StreamState = (
    tokio::sync::mpsc::Sender<openauthority_proto::openauthority::v1::ExecutionEvent>,
    tokio::task::JoinHandle<
        Result<
            tonic::Response<openauthority_proto::openauthority::v1::StreamEventsResponse>,
            tonic::Status,
        >,
    >,
);
...
impl WalAuditSink {
    ...
    async fn replay_wal(&self, ctx: &mut RunCtx) {
        ...
        for event in buffered {
            let proto = openauthority_proto::openauthority::v1::ExecutionEvent::from(event);
    ...
    async fn try_stream_send(stream: Option<&StreamState>, event: &ExecutionEvent) -> bool {
        ...
        let proto = openauthority_proto::openauthority::v1::ExecutionEvent::from(event.clone());
    ...
    async fn try_open_stream(&self) -> Option<StreamState> {
        ...
        let (tx, stream_rx) = tokio::sync::mpsc::channel::<
            openauthority_proto::openauthority::v1::ExecutionEvent,
        >(256);
...
mod tests {
    ...
    fn test_new_stores_fields() {
        let sink = WalAuditSink::new(
            ...
            "/var/lib/openauthority/wal",
            100 * 1024 * 1024,
        );
        assert_eq!(sink.endpoint, "https://audit.example.com");
        assert_eq!(sink.wal_path, PathBuf::from("/var/lib/openauthority/wal"));
