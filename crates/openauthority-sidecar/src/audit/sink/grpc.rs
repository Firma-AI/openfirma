...
impl AuditSink for GrpcAuditSink {
    async fn run(
        ...
        loop {
            tokio::select! {
                Some(event) = rx.recv() => {
                    let proto_event = openauthority_proto::openauthority::v1::ExecutionEvent::from(event);
...
mod tests {
    ...
    fn sample_event_converts_to_proto() {
        ...
        let _proto: openauthority_proto::openauthority::v1::ExecutionEvent = event.into();
    }
}
