...
/// Top-level CLI for `openauthority` binary.
#[derive(Debug, Parser)]
#[command(name = "openauthority")]
#[command(about = "OpenAuthority runtime tools")]
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
}
...
pub enum Command {
    /// Run an agent command through the `OpenAuthority` runtime wrapper.
    Run(RunArgs),
...
/// Arguments for `openauthority run`.
#[derive(Debug, Args)]
pub struct RunArgs {
    ...
    ///
    /// Accepted forms: `<tcp://127.0.0.1:8080>`, `<unix:///run/openauthority-sidecar.sock>`
    #[arg(long)]
...
pub struct ProxyBridgeArgs {
    ...
    /// Upstream host-side Unix socket path exposed by `openauthority run`.
    #[arg(long)]
