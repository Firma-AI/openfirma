...
/// Deterministic identity tuple associated with a `openauthority run` execution.
///
...
impl RunIdentity {
    ...
    pub fn attribution_headers(&self) -> BTreeMap<String, String> {
        ...
        headers.insert(
            "x-openauthority-sandbox-id".to_string(),
            self.sandbox_id.clone(),
        );
        headers.insert(
            "x-openauthority-session-id".to_string(),
            self.session_id.clone(),
        );
        headers.insert("x-openauthority-profile".to_string(), self.profile.clone());
        headers
    }
}
