...
fn resolve_sidecar_ca_cert_path() -> Option<PathBuf> {
    ...
    {
        let path = PathBuf::from(ca_dir).join("openauthority-ca.crt");
        if path.is_file() {
            return Some(path);
        }
    }
    ...
    let cwd_candidate = std::env::current_dir()
        .ok()
        .map(|cwd| cwd.join("openauthority-ca").join("openauthority-ca.crt"));
    let default_candidates = [
        cwd_candidate,
        Some(PathBuf::from("/etc/openauthority/ca/openauthority-ca.crt")),
        Some(PathBuf::from(
            "/var/lib/openauthority/ca/openauthority-ca.crt",
        )),
    ];
...
mod tests {
    ...
    fn injects_sidecar_ca_trust_env_vars() {
        let mut env = BTreeMap::new();
        let cert_path = PathBuf::from("/tmp/openauthority-ca/openauthority-ca.crt");
