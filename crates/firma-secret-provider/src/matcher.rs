    ...
    pub fn compile(spec: &SecretMatcher) -> Result<Self, MatcherError> {
        match spec {
            ...
            SecretMatcher::Regex { pattern } => {
                ...
                let groups = get_groups(&pattern, None);
...
fn rewrite_json(
    ...
    let items = match &matcher.item {
        ...
        Some(selector) => optional_strings(
            selector,
            &root,
            &records,
            "item_selector",
    ...
    let domains = match &matcher.domain {
        ...
        Some(selector) => optional_strings(
            ...
            &records,
            "domain_selector",
            |domain, _, _| validate_domain(domain),
        )?,
    };
...
fn rewrite_regex(
    ...
    for caps in matcher.pattern.captures_iter(text) {
        let value = caps.name(VALUE).ok_or_else(|| MatcherError::MissingGroup {
            missing: VALUE,
            found: get_groups(&matcher.pattern, Some(&caps)).join(", "),
        })?;
        let name = caps
            .name(NAME)
...
fn get_groups<'a>(pattern: &'a Regex, caps: Option<&Captures<'_>>) -> Vec<&'a str> {
    let iter = pattern.capture_names().flatten();
