...
use crate::backend::{LaunchSpec, PrepareRequest, SandboxHandle, build_backend};
use crate::capability::CapabilityLeaseManager;
use crate::config::{
    CaTrustMode, CapabilitySource, MountSpec, ResolvedProfile, SidecarEndpoint, resolve_profile,
};
...
pub fn execute_run(args: &RunInput) -> Result<i32, RunError> {
    ...
    let run_result = (|| {
        ...
        let vscode_state_dir = if should_apply_vscode_shim(&profile, &executable) {
            let state_dir =
                resolve_vscode_state_dir(args.config.as_deref(), &handle_ref.runtime_dir)?;
            let prepared = prepare_vscode_shim(
        ...
        if let Some(state_dir) = vscode_state_dir {
            let handle_mut = handle
                .as_mut()
                .ok_or_else(|| RunError::Internal("sandbox handle missing".to_string()))?;
            ensure_vscode_state_mount(handle_mut, &state_dir);
        }
...
fn should_apply_vscode_shim(profile: &ResolvedProfile, executable: &str) -> bool {
    profile.id == "vscode"
...
#[derive(Debug)]
struct PreparedVscodeShim {
    executable: PathBuf,
    args: Vec<String>,
}

fn prepare_vscode_shim(
    runtime_dir: &Path,
    state_dir: &Path,
    executable: &str,
    args: Vec<String>,
...
fn ensure_vscode_state_mount(handle: &mut SandboxHandle, state_dir: &Path) {
...
#[cfg(windows)]
fn vscode_shim_path(shim_dir: &Path) -> PathBuf {
...
mod tests {
    ...
    fn vscode_shim_creates_runtime_wrapper_and_prepends_path() {
        ...
        let shim = super::vscode_shim_path(&tmp.path().join("bin"));
    ...
    fn vscode_state_dir_uses_config_parent_when_available() {
        ...
        let state_dir = super::resolve_vscode_state_dir(Some(&config_path), &runtime_dir)
            .unwrap_or_else(|e| panic!("{e}"));
    ...
    fn vscode_state_mount_is_added_once() {
        ...
        super::ensure_vscode_state_mount(&mut handle, &state_dir);
        super::ensure_vscode_state_mount(&mut handle, &state_dir);
    ...
    fn vscode_shim_rejects_state_and_window_conflicts() {
        let tmp = tempfile::tempdir().unwrap_or_else(|e| panic!("{e}"));
        let mut env = BTreeMap::new();
        let error = super::prepare_vscode_shim(
            tmp.path(),
