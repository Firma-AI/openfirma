...
/// Addresses of the secret services started by [`pre_bind_gateway`].
///
/// The gateway address is passed to the Sidecar before it starts, while the
/// broker address is injected into shimmed agent processes by [`prepare`].
...
/// Start the secret gateway and broker before the Sidecar starts.
///
/// Returns `None` when the profile lists no secret providers (no gateway needed).
/// Otherwise binds both sockets on a dedicated Tokio runtime and returns their
/// addresses after both listeners are ready.
///
...
/// # Errors
///
/// Returns [`RunError`] if the shim directory cannot be created, the service
/// thread cannot start, or either socket cannot be bound.
pub(super) fn pre_bind_gateway(
    ...
    let gateway_endpoint = server_endpoint(&base, "gateway.sock")?;
    let broker_endpoint = server_endpoint(&base, "broker.sock")?;
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
                ...
                .build()
            {
                Ok(runtime) => runtime,
                Err(error) => {
                    let _ = startup_tx.send(Err(format!("build secret service runtime: {error}")));
                    return;
                }
            };
            runtime.block_on(async move {
                let gateway = match GatewayListener::bind(
                    &gateway_endpoint,
                ...
                .await
                {
                    ...
                    Err(error) => {
                        let _ = startup_tx.send(Err(format!("bind secret gateway: {error}")));
                        return;
                    }
                };
                let broker =
                    match BrokerListener::bind(&broker_endpoint, BrokerConfig::default()).await {
                        Ok(listener) => listener,
                        Err(error) => {
                            let _ = startup_tx.send(Err(format!("bind secret broker: {error}")));
                            return;
                        }
                    };
                ...
                tokio::join!(
                    gateway.serve_forever(Arc::clone(&store)),
                    serve_forever(broker, store, spec_for, None),
                );
            });
        })
        .map_err(|error| RunError::Internal(format!("start secret service thread: {error}")))?;

    let (addr, broker_addr) = startup_rx
        .recv()
    ...
    Ok(Some(BoundGateway { addr, broker_addr }))
}
...
/// or the pre-bound service addresses are unavailable.
pub(super) fn prepare(
    ...
    if !cli_providers.is_empty() {
        ...
        let plan = plan(&shim_bin, &reals, &gateway.broker_addr);
        handle
            .mounts
