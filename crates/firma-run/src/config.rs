...
/// profile/backend pair. Recognized bwrap profiles get a managed baseline;
/// profiles with atomic state stores may use a less restrictive variant. The OS
/// gate (`target_os = "linux"`) is applied separately by the caller.
