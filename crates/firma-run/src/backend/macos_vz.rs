...
use crate::config::{MountSpec, NetworkPolicy};
...
fn vz_structural_mode() -> VzStructuralMode {
    vz_structural_mode_from_flag(crate::config::env_truthy(VZ_STRUCTURAL_NETWORK_ENV))
}

fn vz_structural_mode_from_flag(sandbox_exec_network: bool) -> VzStructuralMode {
    if sandbox_exec_network {
...
impl SandboxBackend for VzBackend {
    ...
    fn prepare(&self, request: &PrepareRequest) -> Result<SandboxHandle, RunError> {
        ...
        if vz_structural_mode() == VzStructuralMode::SandboxExecNetworkDeny {
            tracing::info!(
...
mod tests {
    ...
    use super::{VzStructuralMode, build_sandbox_profile, vz_structural_mode_from_flag};
    ...
    #[test]
    fn structural_mode_follows_sandbox_exec_flag() {
        assert_eq!(
            vz_structural_mode_from_flag(true),
            VzStructuralMode::SandboxExecNetworkDeny
        );
        assert_eq!(
            vz_structural_mode_from_flag(false),
            VzStructuralMode::Compatibility
        );
    }
