...
//!
//! Redaction wraps a sanctioned tool's stdio: placeholders on the tool's
//! **stdin** are rehydrated into real secrets, and secrets on its **stdout** are
//! masked back into placeholders. Both directions are the same operation — find
//! non-overlapping leftmost-longest matches of a pattern set and replace their
//! spans — run incrementally over a stream.
//!
...
/// Direction of a `raw` rewrite over a wrapped tool's stream.
