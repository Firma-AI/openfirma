...
6. **Config mask (Linux `bwrap`).** Firma config is masked from inside the sandbox by default. When `firma.toml` lives in a `.firma/` directory, that whole directory is tmpfs-overlaid so the file is absent and policy bundles and keys beside it are hidden too; an explicit bare config file outside `.firma/` is bound over `/dev/null` instead, because masking its parent could hide the workspace root. The mask covers exactly the set discovery can select: `$HOME/.firma` (the trusted config directory) plus any explicit `--config` / `$FIRMA_CONFIG` file. Because discovery never walks the cwd tree (see below), a `.firma/` an agent plants in the workspace can never be selected for a later run, so it is deliberately left unmasked — masking it would protect nothing. Symlinked `.firma/` directories in the masked set fail closed before launch, because a writable symlink entry could otherwise be swapped for a real config directory mid-run. Since `firma.toml` is loaded once at startup and not hot-reloaded, this closes a read leak (Authority topology / `agent_id`, keys) into the live config. `$HOME/.firma` is masked **even when it does not yet exist**, so an agent can't plant a `~/.firma/firma.toml` for a later run to trust — the write lands in the ephemeral tmpfs, not the host home. The mask is emitted last — after the workspace cwd bind and every profile mount — so it always wins by bwrap's last-write-wins: no mount (a workspace-parent bind, a mount targeting `.firma/`, or one landing inside it) can re-expose `firma.toml`.
...
The sandbox is only as trustworthy as the `firma.toml` that configures it — the config carries the enforcement policy, action mappings, allowlists, Authority/Sidecar endpoints and keys, secret providers, backend selection, and egress/DNS confinement. Whoever controls the selected file controls the guardrails.

For that reason discovery **never walks up from the current directory**. `firma run` resolves `firma.toml` from a single, fixed, user-controlled trusted directory — `~/.firma` (or `%USERPROFILE%\.firma` on Windows), deliberately not the XDG base directory, matching security tools like `~/.ssh` and `~/.aws` — see [Config Discovery](/docs/cli.md#config-discovery). A nearest-`.firma`-wins scan (the git/npm convention) would let a compromised prompt plant `./.firma/firma.toml` in the workspace, or in any writable ancestor, and weaken the agent's own enforcement before it launches. The trust root is a path the **user** controls, not one the agent or repository controls. `--config` and `$FIRMA_CONFIG` remain explicit escapes, read from the launcher's environment — which a sandboxed agent cannot set.
...
| Config (`firma.toml`) mask | **Yes.** `$HOME/.firma` plus any explicit `--config` / `$FIRMA_CONFIG` file is masked by default, so the agent cannot read Authority topology / `agent_id` unless an explicit mount re-exposes it. | **Planned.** |
