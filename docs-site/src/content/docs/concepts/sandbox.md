...
7. **Owned HTTPS CA isolation (Linux `bwrap`).** When `firma run` starts its own Sidecar, it hides the generated CA directory and the effective MITM private-key path, including an explicit `ca_key_path` outside that directory, then exposes only the effective public trust file. A profile mount is rejected if its canonical source is an ancestor, equal to, or below either protected private path. This is intentionally stricter than ordinary mount handling: move the mount source or the explicit key if they overlap, rather than creating a second sandbox route to CA signing material. Trust files supplied for a non-owned Sidecar remain ordinary public inputs and may be symlinks.
