...
OpenFirma is built from source with Cargo. You need a Rust toolchain and `protoc`, but no API keys.
...
The scaffolded `generic` run profile picks a sandbox backend matched to your host, so `firma run` works out of the box without a manual `--backend` override:
...
To override, pass `firma run --backend <name>` or edit `backend` under `[profiles.generic]` in `.firma/firma-run.toml`.
...
2. Edit `.firma/firma.toml`.
   ...
   The preflight settings pin a stable session ID and grant that session access to the GitHub resource used below. The `generic` profile routes traffic through the HTTP proxy Sidecar by default — no extra config needed.
...
`firma run` launches a command inside a sandbox and forces all of its outbound traffic through the enforcement Sidecar. It autostarts a local Sidecar and the local Authority that `firma config` configured in your `firma.toml`, then tears both down when the command exits.
...
The `[authority]` section `firma config` wrote makes `firma run` autostart a local Authority. To use an Authority you already run instead, point at it with `--authority`:

```bash
firma run --authority https://authority.internal:50051 --config .firma/firma.toml -- <command>
```

In CI or any non-interactive context, pass `--no-autostart` to fail with a typed error instead of silently starting a local Authority or Sidecar.
