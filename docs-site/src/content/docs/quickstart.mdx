...
`firma config` scaffolds everything a project needs in one command: a `firma.toml` config file, signing keys, and a set of default Cedar policies.

```bash
firma config
```

The interactive wizard asks a few questions — which agent you run, which provider it talks to, and whether to use a local Authority. The defaults are sensible; press Enter to accept them. The default `agent-local` mode writes an `[authority]` section into `firma.toml`, so `firma run` starts a local Authority without prompting later. The scaffold lands in a `.firma/` directory under your current folder.

If you accepted the installer's prompt to scaffold a project, this is already done.
