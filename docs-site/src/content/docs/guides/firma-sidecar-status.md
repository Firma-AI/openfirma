...
The ID must be the full `sbx_...` sandbox ID generated by Firma. Malformed IDs
are rejected before any marker path is constructed. Find the full ID in
`firma sidecar status --json`, the marker's `metadata.toml`, or the `sandbox_id`
field of an audit event.
