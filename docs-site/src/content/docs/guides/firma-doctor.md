...
| -------------- | ----------------- | ---------- | -------------------------------------------------------------- |
| `--config`     | `FIRMA_STACK_CONFIG` | discovered | Unified `firma.toml`. When unset, auto-discovery uses `$FIRMA_CONFIG` or the trusted config directory (`~/.firma`); selected files must load successfully. |
