...
<Hero
  ...
  tagline="OpenFirma puts a local enforcement point between an agent and the outside world. Every outbound call is intercepted, classified, checked against policy, and recorded — before it leaves the machine."
  actions={[
...
- [Initialize a project](./guides/initialize-a-project/) to scaffold config, keys, and policies in one command with `firma init`.
- [Secure a local coding agent](./guides/secure-a-coding-agent/) to wrap Claude Code, Codex, or Cursor with `firma run`.
- [Run the Sidecar standalone](./guides/run-the-sidecar/) for a minimal setup without `firma run`, pointing agents at the proxy directly.
