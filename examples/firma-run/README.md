...
- `secret-redaction/` shows secret interception & redaction: a `secret_providers` profile entry keeps real secrets out of the agent (vault-CLI intercept + HTTP redact) — being configured is the only authorization needed.
- `secret-redaction-http-vault/` is the HTTP counterpart of `secret-redaction/`: a `type = "http"` `secret_providers` entry, matched by the Sidecar's own HTTP path, to intercept a vault the agent calls directly instead of a CLI it shims.
