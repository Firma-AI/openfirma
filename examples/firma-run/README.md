...
- `secret-redaction-http-vault/` is the HTTP counterpart of `secret-redaction/`: a `type = "http"` `secret_providers` entry plus the same `secret.mediate` Cedar action, evaluated by the Sidecar's own HTTP path to intercept a vault the agent calls directly instead of a CLI it shims.
