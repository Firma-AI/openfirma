...
usage() {
  cat <<'EOF'
Usage:
  scripts/e2e-openauthority-run.sh [--cmd "<shell command>"] [--https-check] [--keep-artifacts]

Description:
  End-to-end local harness for openauthority-run runtime plumbing.
  It:
    1) boots a local opensidecar with temporary config,
    2) runs a sandboxed command via openauthority run,
...
Examples:
  ...
  scripts/e2e-openauthority-run.sh --cmd "cd example_agents/agents_sdk_py && uv run python -m agent.main"
EOF
}
...
WORKDIR="$(mktemp -d /tmp/openauthority-run-e2e.XXXXXX)"
SIDECAR_PID=""
...
cargo run -p opensidecar -- -c "$SIDECAR_CONFIG" --health-bind-addr "$HEALTH_ADDR" -l info >"$SIDECAR_LOG" 2>&1 &
SIDECAR_PID=$!
...
ok "running sandboxed command through openauthority run"
