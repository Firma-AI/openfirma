...
- **Description**: Define an `ExecutionEnvelope` struct with three fields: intent (typed action params via enum — HttpParams, DbQueryParams, ToolUseParams), capability (token reference string), metadata (session ID, agent ID, timestamp, trace ID). No provenance field (deferred — V1 placeholder per PR #5 review). No budget_consumed or risk_score (deferred — unclear computation per PR #5 review). Action params use a typed enum, not a generic map (injection risk per PR #5 review).
- **Acceptance Criteria**: Struct compiles; intent uses typed action params enum; no generic Struct/HashMap for parameters; no provenance, budget_consumed, or risk_score fields
- **Priority**: Must

### FR-6: Execution Context Type

- **Description**: Define an `ExecutionContext` struct representing the attributes used during policy evaluation — agent identity, action, resource, and session metadata. Budget and risk score fields are deferred per PR #5 review — add back when computation mechanisms are designed.
- **Acceptance Criteria**: Struct compiles; fields cover the inputs to Stage 2; can be constructed from an `ExecutionEnvelope`
- **Priority**: Must

### FR-7: Decision Enum

- **Description**: Define a `Decision` enum with three variants: `Allow`, `Deny { reason: DenyReason }`, `Abort { reason: String }`. Define `DenyReason` as a typed enum covering reason codes from the domain design decisions. Include: TokenInvalid, TokenExpired, TokenRevoked, PolicyDenied, ScopeViolation, ToolNotInScope, MalformedRequest, AuthorityUnavailable, PolicyBundleStale, CredentialInjectionFailed, ConnectorTimeout. Deferred: BudgetExceeded, RiskThreshold (corresponding fields removed per PR #5 review — add back when budget/risk mechanisms are designed).
- **Acceptance Criteria**: Enum compiles; all included reason codes are present; `DenyReason` implements `Display` with human-readable messages; deferred codes documented in comments
- **Priority**: Must
