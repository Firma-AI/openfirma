...
| Misconfigured non-protected host passthrough          | `sidecar.mapping.default_protected = false` or permissive allowlisting                                         | Unknown hosts can be forwarded without normalization or policy                     | Partial in V1    | Medium   | Code supports `Passthrough`; config default is hardened (`default_protected = true`) but the escape hatch exists                                     | Keep `default_protected = true`, treat passthrough as exceptional and auditable                   |
...
1. Preserve `sidecar.mapping.default_protected = true` as the secure default and treat any passthrough configuration as an explicit risk acceptance.
